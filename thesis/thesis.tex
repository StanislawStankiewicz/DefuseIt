...
\begin{longtable}{|l|r|r|p{8.0cm}|}
  ...
  \textbf{T04} & 5 & -- & \textit{TBD (wersja robocza).} \\
  \hline
  
  \textbf{T05} & 3 & 100\% & Upływ czasu i detekcja timeout. Odchylenie czasowe $\approx 2$ sekundy względem oczekiwanego limitu (zawsze wcześniej niż deklarowano). Sekwencja dźwiękowa ostrzeżeń w ostatnich 10 sekundach wymagała ręcznej obserwacji, ponieważ synchronizacja z wyświetlaczem czasowym (TM1637) mogła się lokalnie różnić. \\
  \hline
...
Test potwierdzał prawidłowość działania procedury timeout. Przeprowadzono 3 przebiegi, każdy skutkując prawidłowym przekroczeniem limitu czasowego i blokowaniem interfejsów modułów. Obserwacje:
\begin{itemize}
  \item Odchylenie czasowe: $\approx 2$ sekundy wcześniej niż oczekiwane (w każdym przebiegu).
...
\end{itemize}
Status: \textbf{Zaliczone z uwagą}. Odchylenia czasowe mieszczą się w tolerancji akceptowalnej dla aplikacji (\textit{escape room}), gdzie do 2 sekundy wariancji nie stanowi istotnego wpływu na doświadczenie użytkownika.

...
W niniejszym rozdziale przedstawiono wyniki empirycznej weryfikacji warstwy sprzętowej systemu. Program testowy został zaprojektowany w celu walidacji kluczowych parametrów funkcjonalnych, takich jak stabilność protokołów komunikacyjnych, mechanizmy obsługi błędów oraz precyzja odmierzania czasu. Podsumowanie przeprowadzonych scenariuszy testowych zestawiono w Tabeli~\ref{tab:test_results}.
...
\subsection{Szczegółowa analiza wyników}

\subsubsection{Test T01: Powtarzalność procesu inicjalizacji}
Celem testu była weryfikacja stabilności procedury automatycznej adresacji i wykrywania modułów na magistrali I\textsuperscript{2}C. Przeprowadzono 10 cykli zimnego startu systemu, monitorując poprawność identyfikacji wszystkich urządzeń peryferyjnych przez jednostkę centralną (Master). Uzyskano wynik \textbf{10/10}. Wszystkie próby przebiegły pomyślnie — każdy moduł udzielił potwierdzenia ACK w odpowiedzi na ramkę \texttt{CMD\_IDENTIFY}, a po uruchomieniu systemu wszystkie jednostki zgłosiły gotowość do pracy.

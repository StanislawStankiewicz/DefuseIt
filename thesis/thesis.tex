...
\section{Cel i uzasadnienie projektu}
...
Niniejsza praca inżynierska obejmuje i podsumowuje pełen cykl rozwojowy komponentów – od zaplanowania architektury podziałowej i fizycznej budowy obwodów z użyciem technik lutowania oraz druku 3D obudów, poprzez opracowanie algorytmiki oprogramowania, aż do ostatecznych testów obciążeniowych sprzętu. Wyszczególniony wykaz obszarów zadaniowych zrealizowanych w ramach tego dyplomu to:
\begin{itemize}
  ...
  \item montaż i zaprogramowanie w języku C++ modułów: arytmetycznego, labiryntu (wyświetlacz OLED), radiowego (interpretacja alfabetu Morse'a), sekwencyjnego (\textit{Simon Says}) oraz modułu z przełącznikiem dźwigniowym,
...
\begin{longtable}{|c|p{3.8cm}|p{8.2cm}|}
  ...
  \hline
  WF-04          & Obsługa wielu typów modułów          & System obsługuje co najmniej pięć typów modułów: Keypad, Labyrinth, Radio, Simon Says oraz Switch.                                                        \\
  \hline
...
System oparto na architekturze \textit{Master-Slave}, gdzie centralna jednostka koordynuje pracę modułów przez magistralę I$^2$C. Wybór ATmega328P \cite{atmega328p_datasheet} podyktowany był dostępem do sprzętowego kontrolera I$^2$C (TWI) oraz łatwością łączenia z popularnymi peryferiami 5\,V. Zastosowanie języka C++ pozwoliło na programowanie obiektowe z pełną kontrolą nad pamięcią.

% ZDJĘCIE UŻYTKOWNIKA: Tutaj można dodać diagram blokowy architektury całego systemu
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{diagram_klas.png}
  \caption{Schemat blokowy architektury sprzętowej Master-Slave realizowanej przez szynę I$^2$C (opracowanie własne)}
...
\begin{enumerate}
  \item \textbf{Faza wykrywania (\textit{Discovery}):} Wywołanie pętli wysyłkowej z komendą 0x07 (\texttt{CMD\_IDENTIFY}), skanującej adresy od 1 do 126 z zapytaniem o rzetelną odpowiedź ACK sprzętu.
  \item \textbf{Faza propagacji i rejestracji startu:} Przeprowadzenie inicjacyjnej transakcji rozgłaszanej z flagą \texttt{CMD\_START\_GAME}.
  \item \textbf{Faza testowania cyklicznego stanu (\textit{Run Loop}):} Ciągłe odpytywanie modułów o ich stan logiczny, przechwytywanie zdarzeń błędu (\texttt{STATUS\_FAILED}) i sukcesu (\texttt{STATUS\_PASSED}).
\end{enumerate}

\section{Konstrukcja fizyczna i montaż}
Ważnym aspektem projektu było fizyczne wykonanie modułów. Każda stacja została zmontowana na dedykowanej płytce prototypowej, co umożliwiło łatwe wprowadzanie poprawek w trakcie testów. Wszystkie połączenia krytyczne (zasilanie, magistrala I$^2$C) zostały przylutowane w celu zapewnienia stabilności kontaktu pod wpływem drgań mechanicznych.

W procesie produkcji wykorzystano:
\begin{itemize}
  \item \textbf{Drukowanie 3D}: Obudowy dla modułów zostały zaprojektowane w programie typu CAD i wydrukowane z materiału PLA, co zapewniło estetyczny wygląd i ochronę elektroniki.
  \item \textbf{Montaż mechaniczny}: Przyciski, enkoder oraz wyświetlacze zostały osadzone w otworach montażowych, co zapobiega ich przesuwaniu się podczas interakcji z użytkownikiem.
  \item \textbf{Zasilanie}: Całość systemu zasilana jest napięciem 5\,V ze wspólnego źródła, co eliminuje potrzebę stosowania oddzielnych baterii dla każdego modułu.
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{diagram_klas.png}
...
Wspólnym elementem architektury wszystkich modułów jest mechanizm wstrzykiwania callbacków --- każdy moduł przekazuje w konstruktorze klasy \texttt{Slave} wskaźniki na swoje funkcje \texttt{gameLoop()} oraz \texttt{resetState()}, realizując wzorzec \textit{Inversion of Control}. Dzięki temu klasa \texttt{Slave} wywołuje logikę specyficzną dla danego modułu bez znajomości jej implementacji.
...
W niniejszym rozdziale przedstawiono technologie, narzędzia oraz biblioteki wykorzystane w projekcie. Dla każdego komponentu opisano jego rolę oraz sposób wykorzystania. Realizacja projektu opierała się na sprawdzonych rozwiązaniach otwartoźródłowych (ang. \textit{open-source}), co pozwoliło skupić się na logice gry zamiast na tworzeniu sterowników od podstaw.
...
Wybór tego układu podyktowany był trzema czynnikami. Po pierwsze, jest on sercem popularnej platformy Arduino Uno, co daje dostęp do ogromnej liczby bibliotek oraz materiałów edukacyjnych. Po drugie, oferuje natywną obsługę peryferiów pracujących w standardzie 5\,V, co eliminuje potrzebę stosowania konwerterów napięć. Po trzecie, posiada sprzętowy kontroler I$^2$C (TWI), który jest fundamentem komunikacji w tym systemie.

W projekcie układy pracują pod kontrolą wewnętrznego oscylatora RC skonfigurowanego na częstotliwość 8\,MHz. Wykorzystanie wbudowanego źródła sygnału zegarowego, zamiast zewnętrznego kwarcu, obniża złożoność i koszt obwodu, a jednocześnie pozwala na pracę w poszerzonym zakresie napięć zasilania od 2.7\,V do 5.5\,V. Choć wbudowany oscylator charakteryzuje się tolerancją wahań do $\pm10\%$ (w stałych warunkach pokojowych błąd oscyluje w granicach $\pm$1--2\%), odchylenia te są pomijalne w kontekście działania gry oraz obsługi magistrali I$^2$C. Konfigurację niskopoziomową mikrokontrolera (ustawienie fuzji \textit{fuses}, m.in. \texttt{LFUSE: 0xE2}) przeprowadzono bezpośrednio przy użyciu narzędzia wiersza poleceń \texttt{avrdude}, co umożliwiło precyzyjne sterowanie parametrami pracy układu bez użycia dodatkowych nakładek systemowych. Bezpośrednie wgrywanie oprogramowania przez złącze ISP pozwoliło całkowicie wyeliminować bootloader, dzięki czemu kod programu ma do dyspozycji pełne 32\,KB pamięci Flash. Jest to istotna optymalizacja względem standardowej platformy Arduino Uno, w której część pamięci programu stale zajmuje program rozruchowy.
...
W ramach projektu cały proces programowania układów realizowano przy pomocy oprogramowania \texttt{avrdude} za pośrednictwem zewnętrznego programatora sprzętowego USBasp, wykorzystując funkcję \textit{Upload Using Programmer} (wgrywanie za pomocą programatora). Na potrzeby produkcji wielu egzemplarzy urządzeń brzegowych przygotowano autorski adapter sprzętowy wpięty do portu USB, wyposażony w profesjonalną podstawkę typu ZIF (ang. \textit{Zero Insertion Force}) z charakterystyczną dźwignią, co umożliwiło szybką i bezpieczną wymianę mikrokontrolerów bez ryzyka uszkodzenia pinów. Konstrukcja programatora oparta jest na przylutowaniu magistrali sygnałowej ISP (styki: 5\,V, GND, MOSI, MISO, RST, SCK) bezpośrednio do gniazda procesora, co zapewnia stabilny proces wgrywania wsadów binarnych bezpośrednio do pamięci Flash.
...
Protokół I$^2$C został wybrany jako magistrala komunikacyjna systemu ze względu na minimalizację wymaganego okablowania --- do połączenia dowolnej liczby urządzeń wystarczają zaledwie dwie linie sygnałowe (SDA i SCL) oraz wspólne zasilanie. W porównaniu z alternatywami, takimi jak SPI (wymagający dodatkowej linii CS dla każdego urządzenia) czy UART (ograniczony do komunikacji punkt-punkt), I$^2$C stanowi optymalny kompromis między prostotą połączeń a skalowalnością systemu rozproszonego.

W projekcie biblioteka \texttt{Wire.h} stanowi fundament autorskiej biblioteki \texttt{ModuleComms}, realizując fizyczną warstwę transportową komunikacji między jednostką Master a modułami Slave.
...
W projekcie biblioteki Adafruit wykorzystano w modułach Labyrinth oraz Radio do obsługi monochromatycznych wyświetlaczy OLED o rozdzielczości 128x64 pikseli. Warstwa \texttt{Adafruit\_GFX} umożliwia rysowanie prymitywów graficznych (prostokątów, okręgów, glifów tekstowych) oraz renderowanie bitmap przechowywanych w pamięci Flash za pośrednictwem dyrektywy \texttt{PROGMEM}, co pozwala na obsługę dużych zasobów graficznych bez obciążania ograniczonej pamięci SRAM.

\section{Biblioteka wyświetlacza LiquidCrystal}
LiquidCrystal\_I2C jest modyfikacją oficjalnej biblioteki \texttt{LiquidCrystal} środowiska Arduino, rozszerzoną o obsługę komunikacji przez magistralę I$^2$C za pośrednictwem ekspandera portów PCF8574. W tradycyjnym ujęciu, panel LCD wymaga podłączenia do 6 przewodów sygnałowych, trwale rezerwujących cyfrowe piny mikrokontrolera. Zastosowanie konwertera I$^2$C redukuje tę liczbę do dwóch linii współdzielonych z pozostałymi urządzeniami na magistrali.

Wybór wariantu I$^2$C tej biblioteki wynikał bezpośrednio z architektury systemu --- moduł arytmetyczny, w którym jest ona wykorzystywana, korzysta już z magistrali I$^2$C do komunikacji z jednostką nadrzędną. Dodanie wyświetlacza LCD na tę samą szynę wymagało jedynie zmiany adresu urządzenia, bez zajmowania dodatkowych pinów mikrokontrolera. W ramach projektu biblioteka służy do wyświetlania treści zadań matematycznych oraz wprowadzanych przez użytkownika odpowiedzi na panelu alfanumerycznym LCD 16x2.
...
W projekcie wyświetlacz TM1637 pełni rolę głównego wskaźnika upływającego czasu gry w formacie MM:SS, informując uczestników o zbliżającym się jego końcu.

\section{Biblioteka interfejsu SPI -- SPI.h}
...
Protokół SPI został zastosowany w module Labyrinth zamiast I$^2$C ze względu na znacząco wyższą przepustowość, niezbędną do wydajnego przesyłania buforów ramki obrazu o rozmiarze 1\,KB (128x64 pikseli monochromatycznych) do wyświetlacza OLED. Przesyłanie tej ilości danych przez magistralę I$^2$C, współdzieloną z komunikacją systemową, mogłoby negatywnie wpłynąć na responsywność całego systemu.
...
W protokole komunikacyjnym każde polecenie i każdy status to pojedynczy bajt w notacji szesnastkowej. Komendy wywołujące akcję po stronie serwera to np. \texttt{CMD\_START\_GAME} czy \texttt{CMD\_IDENTIFY}, a flagi stanu modułu to \texttt{STATUS\_UNSOLVED}, \texttt{STATUS\_PASSED} i \texttt{STATUS\_FAILED}. Dzięki temu zamiast przesyłać przez magistralę pełne tekstowe identyfikatory poleceń wystarczy pojedynczy bajt --- co zmniejsza liczbę przesyłanych danych i zwiększa przepustowość komunikacji.
\begin{lstlisting}[language=C++, caption={Definicje protokołu transportowego I$^2$C w bibliotece ModuleComms}]
...
\begin{lstlisting}[language=C++, caption={Interfejs główny platformy nadzorującej (ModuleComms.h)}]
class Master {
public:
...
Integracja modułu z biblioteką polega na przekazaniu dwóch funkcji: \texttt{gameLoopFn} (logika zagadki) oraz \texttt{resetStateFn} (powrót do stanu początkowego). Jest to realizacja wzorca inwersji sterowania (\textit{Inversion of Control}) --- moduł dostarcza kod, a biblioteka decyduje o momencie jego wywołania. Programista musi jedynie wywołać \texttt{slave.slaveLoop()} w głównej pętli Arduino.

\begin{lstlisting}[language=C++, caption={Uproszczona definicja interfejsu polimorficznego do węzłów łamigłówek (ModuleComms.h)}]
...
Jednostka nadrzędna stanowi centralny punkt decyzyjny całego omawianego systemu. Jako jedyna z peryferiów, nie implementuje ona podrzędnego interfejsu zagadek, lecz służy za główny zegar odliczający i dystrybutor globalnych ustawień gry. Fizyczny sprzęt centrali rozbudowano o poczwórny wyświetlacz 7-segmentowy (\texttt{TM1637}), pełniący rolę głównego wskaźnika czasu, oraz dodatkowy, 2-cyfrowy wyświetlacz 7-segmentowy obsługiwany przez rejestry przesuwne. Ten drugi służy do prezentacji wylosowanego ziarna pseudolosowości (zmiennej \texttt{version}), determinującego logikę pozostałych modułów. Interfejs wyposażono również w przycisk Start, przełącznik dźwigniowy oraz przetwornik piezoelektryczny (buzzer) do sygnalizacji dźwiękowej.
...
\begin{lstlisting}[language=C++, caption={Nieblokująca obsługa przyspieszonego upływu czasu ostrzegawczego (Master.ino)}]
void handleBeeps() {
...
\begin{lstlisting}[language=C++, caption={Programowe ubezpieczenie przed przypadkowym restartem gry z licznikiem czasowym operacji (Master.ino)}]
void handleSwitch() {
...
\begin{lstlisting}[language=C++, caption={Procedura odpytywania stacji na nasłuchu I$^2$C (Master.ino)}]
void checkModules() {
...
\begin{lstlisting}[language=C++, caption={Główna pętla logiczna modułu radiowego (Radio.ino)}]
void gameLoop() {
...
\begin{lstlisting}[language=C++, caption={Deklaracja haseł telegraficznych i częstotliwości w pamięci Flash (Radio.ino)}]
...
Skompilowany program dla tego modułu zajmuje 15326 bajtów (47\%) pamięci Flash oraz 692 bajty (33\%) pamięci SRAM. Taki podział zasobów gwarantuje stabilną pracę układu ATmega328P nawet przy intensywnej komunikacji na magistrali I$^2$C.

\subsection{Moduł labiryntu} Stanowi przestrzenną łamigłówkę nawigacyjną, w której gracz porusza się wewnątrz wirtualnego labiryntu za pomocą trzech przycisków (ruch na wprost oraz obroty w prawo lub w lewo). Interfejs graficzny oparto na wyświetlaczu OLED 128x64 pikseli. Co istotne, w przeciwieństwie do innych modułów, system ten komunikuje się z ekranem za pomocą szybkiej magistrali SPI (piny \texttt{MOSI}, \texttt{CLK}, \texttt{CS}, \texttt{DC}, \texttt{RST}). Wybór tego standardu zamiast I$^2$C podyktowany był dwoma czynnikami: wyższą przepustowością niezbędną do odświeżania pełnoekranowych bitmap oraz uniknięciem konfliktu na jedynym sprzętowym interfejsie I$^2$C mikrokontrolera, który musi pracować w trybie Slave na potrzeby komunikacji systemowej.
...
\begin{lstlisting}[language=C++, caption={Główna pętla logiczna modułu labiryntu (Labyrinth.ino)}]
void gameLoop() {
...
\begin{lstlisting}[language=C++, caption={Główna pętla logiczna modułu sekwencyjnego (SimonSays.ino)}]
void gameLoop() {
    switch (currentState) {
...
Implementacja ta pozwala na zachowanie ciągłej łączności z jednostką Master przy jednoczesnej obsłudze animacji diod oraz sygnałów dźwiękowych. Program zajmuje jedynie 7812 bajtów (24\%) pamięci Flash oraz 480 bajtów (23\%) pamięci SRAM, co gwarantuje stabilną pracę układu bez ryzyka przepełnienia pamięci operacyjnej.

\begin{lstlisting}[language=C++, caption={Dynamiczne przypisywanie układu klawiszy w zależności od liczby błędów (SimonSays.ino)}]
...
\begin{lstlisting}[language=C++, caption={Główna pętla logiczna modułu arytmetycznego (Keypad.ino)}]
void gameLoop() {
...
\begin{lstlisting}[language=C++, caption={Nieblokujący odczyt klawiatury z programowym debouncingiem (Keypad.ino)}]
char scanKeypad() {
...
Implementacja modułu zajmuje 9344 bajty (28\%) pamięci Flash oraz 692 bajty (33\%) pamięci SRAM. Zrezygnowano z bibliotek blokujących (\texttt{delay}), co zapewnia płynną współpracę z magistralą I$^2$C i jednostką Master nawet podczas oczekiwania na wprowadzenie przez gracza kolejnych cyfr wyniku.
...
\begin{lstlisting}[language=C++, caption={Główna pętla logiczna modułu z przełącznikiem (Switch.ino)}]
void gameLoop() {
...
\begin{lstlisting}[language=C++, caption={Wyznaczanie poprawnej procedury działania w oparciu o stan gry (Switch.ino)}]
...
Moduł ten cechuje się najmniejszym zapotrzebowaniem na zasoby w całym systemie ze względu na brak wyświetlaczy graficznych. Skompilowany program zajmuje 5452 bajty (16\%) pamięci Flash oraz 545 bajtów (26\%) pamięci SRAM.
...
\begin{itemize}
  ...
  \item \textbf{Pomiar zużycia pamięci:} Wykorzystanie pamięci Flash i SRAM utrzymano na bezpiecznym poziomie dzięki alokacji danych statycznych w pamięci programu (dyrektywa \texttt{PROGMEM}). Przykładowo, moduł Labiryntu wykorzystuje ok. 46\% pamięci Flash i 39\% SRAM.
\end{itemize}
...
Celem pracy było zaprojektowanie modularnego systemu zadań logicznych na mikrokontrolerach ATmega328P. Projekt potwierdził, że architektura Master-Slave i magistrala I$^2$C pozwalają na stworzenie stabilnego i skalowalnego systemu. Oprogramowanie umożliwia koordynację wielu modułów przy zachowaniu płynności interfejsu.

Projekt wykazał znaczenie optymalizacji pamięci (2\,KB SRAM). Wykorzystanie pamięci Flash (\texttt{PROGMEM}) oraz lekkiego protokołu komunikacyjnego pozwoliło na realizację złożonych zadań bez przekraczania limitów sprzętowych.

...
Niniejszy rozdział zawiera szczegółową ocenę systemu DefuseIt z dwóch perspektyw: \textbf{(1)} analizy ograniczeń zasobów sprzętowych mikrokontrolerów ATmega328P oraz \textbf{(2)} empirycznej weryfikacji poprawności komunikacji I$^2$C, niezawodności logiki modułów i precyzji odmierzania czasu. Obydwie części uzupełniają się wzajemnie — pierwsze sekcje pokazują, w jakich warunkach hardware'owych operuje system, podczas gdy testy demonstrują spełnienie wymagań funkcjonalnych bez przekroczenia limitów sprzętowych.

Specyfika mikrokontrolerów AVR wymusza odmienną metodykę testowania niż w przypadku komputerów PC. Základową weryfikacją jest pomyślna kompilacja kodu (\texttt{avr-gcc}), która sprawdza nie tylko składnię, ale i zajętość pamięci Flash oraz SRAM — raporty kompilatora pozwalają wstępnie upewnić się, że program będzie mieścił się w dostępnych limitach. Dodatkowo, do podglądu komunikatów diagnostycznych w czasie rzeczywistym wykorzystywano płytkę Arduino Uno R4 WiFi jako przekaźnik UART-USB, monitorując stany wewnętrzne (adresy I$^2$C, statusy modułów, wartości timerów). Uzupełnieniem weryfikacji kompilacyjnej były testy integracyjne przeprowadzone na fizycznym prototypie, obejmujące zarówno walidację wydajności kanału I$^2$C, jak i pomiary rzeczywistego zużycia pamięci w warunkach operacyjnych.
...
Celem testu była weryfikacja stabilności procedury automatycznej adresacji i wykrywania modułów na magistrali I$^2$C. Przeprowadzono 10 cykli zimnego startu systemu, monitorując poprawność identyfikacji wszystkich urządzeń peryferyjnych przez jednostkę centralną (Master). Uzyskano wynik \textbf{10/10}. Wszystkie próby przebiegły pomyślnie — każdy moduł udzielił potwierdzenia ACK w odpowiedzi na ramkę \texttt{CMD\_IDENTIFY}, a po uruchomieniu systemu wszystkie jednostki zgłosiły gotowość do pracy.

\subsubsection{Test T02: Obsługa stanów awaryjnych}
W ramach testu sprawdzono zdolność jednostki Master do rejestracji i dystrybucji zmiennej stanu błędu (\texttt{mistakeCount}) po wystąpieniu zdarzenia \texttt{STATUS\_FAILED} w dowolnym węźle. Wszystkie testy przebiegły pomyślnie, a system poprawnie zareagował na symulowane błędy, aktualizując stan globalny i przekazując informację do pozostałych modułów.
...
Pomiary wykazują, że system utrzymuje stałą częstotliwość odpytania każdego z modułów na poziomie ~17 Hz, co odpowiada okresowi cyklu około 57–58 ms. Czas odkrywania modułów na magistrali I\textsuperscript{2}C wynoszący średnio 115 ms jest akceptowalny dla zastosowania o charakterze interaktywnym, gdzie elementy UI aktualizują się co kilkaset milisekund. Wynik ten potwierdza efektywność implementacji procedury \texttt{discoverModules()} w kodzie modułu nadrzędnego.

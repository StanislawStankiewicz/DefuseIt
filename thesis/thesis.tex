...
\section{Wyniki testów sprzętowych}
W niniejszej sekcji zebrano wyniki praktycznych testów Hardware'owych przeprowadzonych na systemie. Testy zostały zaplanowane tak, aby weryfikować kluczowe aspekty systemu (stabilność komunikacji, obsługę błędów, czasomierz, debouncing) bez konieczności ciągłego reflashowania oprogramowania na mikrokontrolerach. Poniższa tabela (Tabela~\ref{tab:test_results}) zawiera podsumowanie wyników dla każdego z ośmiu zaplanowanych testów.

\begin{longtable}{|l|r|r|p{8.0cm}|}
  \caption{Wyniki testów sprzętowych systemu modularnego} \label{tab:test_results} \\
  \hline
  \textbf{Test} & \textbf{n} & \textbf{Skut. [\%]} & \textbf{Uwagi} \\
  \hline
  \endfirsthead
  ...
  \textbf{T01} & 10 & 100\% & Powtarzalne rozruchowanie systemu -- wszystkie moduły poprawnie zidentyfikowane na magistrali I\textsuperscript{2}C. Walidacja przeprowadzona poprzez wizualną weryfikację świecenia diod LED na modułach. \\
  \hline
  
  \textbf{T02} & 10 & -- & \textit{TBD (wersja robocza).} \\
  \hline
  
  \textbf{T03} & 10 & 100\% & Obsługa błędów pojedynczych modułów. Licznik błędów na jednostce Master inkrementuje się prawidłowo. Zaobserwowane anomalii: (1) Dioda na module SimonSays pozostaje zapalona po zatrzymaniu animacji; (2) Przełącznik Master nie reaguje na zmianę stanu tuż po fali błędu. Oba zjawiska wymagają weryfikacji logiki przywracania stanu. \\
  \hline
  
  \textbf{T04} & 5 & 100\% & Win path consistency. Wszystkie moduły zostały rozwiązane poprawnie, a zwycięstwo nastąpiło dopiero po zgłoszeniu stanu PASS przez każdy węzeł. \\
  \hline
  
  \textbf{T05} & 5 & 100\% & Upływ czasu i detekcja timeout. W pięciu przebiegach odchylenie wyniosło odpowiednio około 2 s, 2 s, 2 s, 5 s i 6 s względem oczekiwanego limitu, zawsze po stronie wcześniejszego zakończenia. Sekwencja dźwiękowa ostrzeżeń w ostatnich 10 sekundach wymagała ręcznej obserwacji, ponieważ synchronizacja z wyświetlaczem czasowym (TM1637) mogła się lokalnie różnić. \\
  \hline
  
  \textbf{T06} & 3 & -- & \textit{TBD (wersja robocza).} \\
  \hline
  
  \textbf{T07} & 20 & -- & \textit{TBD (wersja robocza).} \\
  \hline
  
  \textbf{T08} & 1 (30 min) & -- & \textit{TBD (wersja robocza).} \\
  \hline
  
\end{longtable}

\subsection{Szczegółowe wyniki testów}

\subsubsection{Test T01: Powtarzalność rozruchowania}
Test potwierdzał stabilność procedury automatycznego wykrywania modułów na magistrali I\textsuperscript{2}C. Przeprowadzono 10 cykli włączenia zasilania systemu, obserwując czy wszystkie moduły są konsekwentnie wykrywane przez jednostkę Master. Wynik: \textbf{10/10 sukcesów}. Wszystkie skanowane moduły udzieliły potwierdzenia ACK na polecenie \texttt{CMD\_IDENTIFY}.

\subsubsection{Test T02: Sanacja poleceń Start/End}
Test weryfikuje czyszczenie stanu wszystkich modułów po poleceniu \texttt{CMD\_END\_GAME}. Zaobserwowana anomalia wskazuje na to, że niektóre moduły (szczególnie radiowy i labiryntu) nie powracają do pełnego stanu bezczynności. Przyczyna: domniemane opóźnienie w procedurze \texttt{resetState()} lub brak kompletnego wyczyszczenia stanu grafiki wyświetlaczy. Status: \textit{W trakcie diagnozowania.}

\subsubsection{Test T03: Obsługa pojedynczych błędów}
Test potwierdzał, że Master poprawnie rejestruje i propaguje licznik błędów (\texttt{mistakeCount}) do wszystkich węzłów po zgłoszeniu statusu \texttt{STATUS\_FAILED} przez jeden z modułów. Wynik: \textbf{10/10 sukcesów}. Zaobserwowane anomalii:
\begin{itemize}
  \item Dioda LED modułu SimonSays czasami pozostaje zapalona po zatrzymaniu sekwencji.
  \item Przełącznik dźwigni na jednostce Master nie reaguje niezwłocznie po zdarzeniu błędu.
\end{itemize}
Rekomendacja: Weryfikacja czasomierza odświeżania w funkcji \texttt{gameLoop()} modułu SimonSays oraz sprawdzenie priorytetu przetwarzania zdarzenia przełącznika na Master.

\subsubsection{Test T04: Konsystencja ścieżki zwycięstwa}
Test potwierdzał, że zwycięstwo jest sygnalizowane wyłącznie po poprawnym rozwiązaniu wszystkich modułów. Przeprowadzono 5 przebiegów i w każdym przypadku system zakończył grę dopiero po zgłoszeniu stanu PASS przez wszystkie węzły. Wynik: \textbf{5/5 sukcesów}.

\subsubsection{Test T05: Ścieżka timeout}
Test potwierdzał prawidłowość działania procedury timeout. Przeprowadzono 5 przebiegów, każdy skutkując prawidłowym przekroczeniem limitu czasowego i blokowaniem interfejsów modułów. Obserwacje:
\begin{itemize}
  \item Odchylenie czasowe: około 2 s, 2 s, 2 s, 5 s i 6 s wcześniej niż oczekiwane.
  \item Sekwencja akustyczna ostrzeżeń (ostatnie 10 sekund) działała prawidłowo, choć jej synchronizacja z wyświetlaczem TM1637 mogła się lokalnie różnić o 1--2 sekundy.
\end{itemize}
Status: \textbf{Zaliczone z uwagą}. Odchylenia czasowe pozostają roboczą obserwacją do dalszej weryfikacji synchronizacji ostrzeżeń i licznika czasu.
